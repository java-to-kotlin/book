...
Now our object-oriented implementation has gained an `invoke` method to fit in with the functional approach.
...
Objects have that advantage over [.keep-together]#functions—they# get to name their methods (`send(email)` is obviously different than `delete(email)`)—at the expense of also having to name the type of the object or [.keep-together]#interface#.
...
Converting to Kotlin and applying the refactorings introduced in Chapters <<functions-to-extension-functions,10>> and <<streams-to-sequences,13>> gives:
