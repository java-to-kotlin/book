...
All these techniques suffer from the drawback that the calling code is able, to a greater or lesser extent, to ignore that an error occurred.
...
Errors::
Subclasses of `java.lang.Error` are reserved for failures so severe that the JVM can no longer guarantee the correct functioning of the runtime.
...
****
This is an interesting case and shows why error handling is so hard.
...
The net result, though, is that the old standard Java API declares checked exceptions (in particular `IOException`) that the new standard API (in particular streams) forces developers to deny.
...
Statically typed functional programming languages often reject exceptions in favor of another error handling technique based on _Either Types_.
...
We'll share your pain by pairing with you in the worked example later.
...
`RuntimeExceptions` are still good for signaling when we have made an error as programmers: `IndexOutOfBounds` and the like.
...
Plenty of other result types are available, but for this book we will use https://github.com/fork-handles/forkhandles/tree/trunk/result4k[Result4k], not coincidentally written by Nat.
...
It can also offer operations, such as `onFailure` and `recover`, that would not make sense on `Either`.
...
The HTTP protocol returns 4xx errors when a request is unsuccessful because the request was wrong in some way, 5xx errors when a request couldn't be processed for server-related reasons; 2xx status codes are success cases; and 1xx and 3xx codes are used to signal an ongoing interaction.
...
Without it, a Java implementation of `Customers` that does throw `DuplicateException` cannot implement the method.
...
We'll start with `CustomerRegistrationHandler`, converting it to Kotlin first:
...
`Result::recover` is a Result4k extension function that unwraps the result if it is `Success`, otherwise returning the result of mapping the failure's `reason`:
...
We do this using `Result::mapFailure` (part of Result4k) to convert `DuplicateException` to `Duplicate`.
...
Now that we have no callers of `register`, we can finally remove it and rename `registerToo` to `register`, ending up with exception-free Kotlin.
...
This has been a long chapter, but its length is in proportion to its importance.
...
We can mitigate the effects of both by actions and errors by moving them as close as we can to the entry points to our system, so that they taint the least code.
