...
Put the string in a message and ask another object representing the console to print it for you: `System.out.println(myString)`.
...
_Receiver_, by the way, is the term that Kotlin uses for the object named `this` in an extension function (or normal method): the object that receives messages.
...
Although we can't call extension functions as normal functions in Kotlin, we can assign them to normal function references. So the following compiles:
...
Extensions really come into their own when we work with optional data.
...
Kotlin's _scoping functions_, such as `let`, `apply`, and `also`, can help here.
...
Here the `?.` ensures that `let` is only called when the customer is not `null`, meaning that the lambda parameter, `it`, is never null, and can be passed to the function within the lambda body.
You can think of `?.let` as a safe-call operator for (single) arguments.
...
Note that we have to use the safe-call operator to dereference `this` inside the extension.
...
Just as with normal functions, extensions can have generic parameters, and things become really interesting when the receiver is generic.
...
You can see that we haven't gone to town on these, because marketing do have a habit of changing their minds, but in essence the report needs to list the customers who have a score of 10 or more, sorted by spend, with a final total line.
...
<<performing-io-to-passing-data>> and <<exceptions-to-values>> will finish off this refactoring, looking at the use of the reader and writer.
...
That may change when we continue refactoring this code in <<performing-io-to-passing-data>>. Let's see, shall we?
