...
.Source Code
****
We have published the source code we show here in a public Git repository.
Details are on https://java-to-kotlin.dev[the book's website].
...
To follow along in Git, you need to check out the repository and then step through the commits.
If you want to do that, we provide instructions on the book's website.
However, we have tried to show everything you need to make sense of the code here in print.
It helps to imagine that the last code sample shown is in the active editor pane as we talk about the changes we are about to make.
****
...
It has replaced our `EmailAddress.java` file with `EmailAddress.kt` in the same directory, though:
...
Java classes that follow the standard coding conventions always define their elements in the same order: class name, superclass, interfaces, and then, within the class body, fields, constructors, and methods.
...
A Kotlin class definition has a header part with the class name, primary constructor (which can contain parameters and/or property definitions), superclass (which may also be the call to the superclass's constructor), and interfaces.
...
One place where Kotlin is noticeably less succinct than Java is where it uses companion objects to host static state and methods, in this case `parse()`.
...
So we'll leave the `Money` class for now and come back to it in <<functions-to-operators>> to give it a thorough makeover.
