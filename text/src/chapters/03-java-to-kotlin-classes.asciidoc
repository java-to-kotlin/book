...
<5> The class implements the `equals` and `hashCode` methods to ensure that two [.keep-together]#++EmailAddress++# values with equal fields compare as equal.
...
Declaring((("domain field"))) the `domain` property causes the compiler to generate a private `domain` field and a `getDomain()` method so that Java code can still call it.
...
Today, we((("transformations", "methods returning a transformation of the receiver"))) avoid the "set" prefix for methods that do not mutate their receiver and instead use names that emphasize when the method returns a transformation of the receiver.
