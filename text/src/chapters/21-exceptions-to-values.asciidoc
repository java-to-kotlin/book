...
Let's change ++String.to&#x200b;Custo&#x2060;merData()++ to return a `Result` rather than nullable:
...
`toHighValueCustomerReport` signals the errors that it knows about (failure to parse) and how they are represented (subclasses of [.keep-together]#++ParseFailure++#) through its `onErrorLine` parameter.
++IOException++s are not its responsibility.
It is `main` that is passing an I/O-backed `Sequence` into `toHighValueCustomerReport`, so `main` should be aware that `toHighValueCustomerReport` can therefore fail with an [.keep-together]#++IOException++# and deal with it accordingly.
Let's add that code:
