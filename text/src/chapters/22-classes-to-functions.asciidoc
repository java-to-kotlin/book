...
But wait!
Looking at the code, we realize that if they want to specify their own field names for ++read&#x200b;Ta&#x2060;ble++, they can do this by overriding the default `headerProvider` in `readTable`.
...
Neither the header parsing nor the body parsing really want to depend on the details of how the splitting should happen, so let's abstract that behind a function `(String)` [.keep-together]#++-> List<String>++#.
...
It feels like there should be an automated refactor for this, but nothing works at the time of [.keep-together]#writing#:
...
Moving on, the code to create an individual `Measurement` looks identical in both [.keep-together]#examples#:
