...
For example, a `Route` contains a list of `Journey`, each of which has a cost, and `Accommodation` has a room rate, a number of nights, and additional costs such as meals and hotel services.
...
First, it introduces the possibility of https://oreil.ly/PeqKs[aliasing errors].
...
A `CostSummary` is a mutable POJO (as described in <<beans-to-values>>) that holds a list of `CurrencyConversion`, from local currencies to the traveler's preferred currency.
...
The `summarise` method constructs a `CostSummary` using a source of exchange rates to convert local costs to the traveler's preferred currency:
...
`CostCurrencyCalculator` is a so-called "doer class."
...
Travelator does more with exchange rates and the traveler's preferred currency than summarize costs.
