...
When all uses of `CostSummaryCalculator` in the application are using our new [.keep-together]#++summarise++# method, we can flatten the calculation of the `currencyTotals` and `CostSummary` into that method, the one that currently uses a local to do the job:
